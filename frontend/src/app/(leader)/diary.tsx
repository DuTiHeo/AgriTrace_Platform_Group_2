...
export default function DiaryScreen() {
  ...
  const { refresh, loading, error } = useReports();
  const [query, setQuery] = useState('');
  useFocusEffect(useCallback(() => { void refresh(); }, [refresh]));
  const visible = diaries.filter(d => d.name.toLocaleLowerCase('vi').includes(query.toLocaleLowerCase('vi')));
  return (
    <Screen title="Nhật ký canh tác">
      <Text style={s.muted}>{diaries.length} nhật ký từ tổ của bạn</Text>
