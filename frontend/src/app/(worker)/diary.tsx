    ...
    <ScrollView keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag" contentContainerStyle={s.content}>
      <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
        <View style={{ gap: 16, flexGrow: 1 }}>
          ...
          {!visible.length && <View style={s.card}>
          ...
          {visible.map(report => <View key={report.id} style={s.card}>
            <View style={s.row}><Text style={[s.section, { flex: 1 }]}>{report.taskTitle}</Text><Text style={s.chip}>{reviewLabel(report)}</Text></View>
            <Text style={s.label}>{user?.full_name ?? 'Bạn'}</Text>
