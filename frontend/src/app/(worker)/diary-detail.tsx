...
export default function WorkerDiaryDetailScreen() {
  ...
  return <SafeAreaView style={s.page} edges={['top', 'bottom']}>
    ...
    <ScrollView contentContainerStyle={s.content}>
      {report ? <>
        <View style={s.card}>
          <Text style={s.chip}>{reviewLabel(report)}</Text>
          <Text style={s.title}>{report.taskTitle}</Text>
          <Text style={s.muted}>Người thực hiện</Text><Text style={s.label}>{user?.full_name ?? 'Bạn'}</Text>
      ...
      </> : <View style={s.card}><Text style={s.muted}>Không tìm thấy nhật ký. Vui lòng quay lại danh sách.</Text></View>}
    </ScrollView>
  </SafeAreaView>;
}
